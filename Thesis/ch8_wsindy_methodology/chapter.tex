...
Sparsity-promoting regression originates with the Lasso~\cite{tibshirani1996lasso}; in the PDE-discovery context, the same principle underpins both SINDy's sequential thresholded least squares~\cite{brunton2016sindy} and the SR3 relaxation~\cite{zheng2019sr3}.
...
\paragraph{Library-structure biases (constrain what can be discovered).}
...
\paragraph{Post-fit filters (constrain regression output, not library).}
...
\paragraph{Preprocessing biases (act before regression).}
