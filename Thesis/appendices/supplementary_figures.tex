...
The following density-snapshot grid complements the representative gas
...
In practice, both MVAR and LSTM preserve total mass to machine precision
($|M(t)/M_0 - 1| < 10^{-14}$) throughout the forecast horizon, after the
inverse POD transform and the adopted mass-postprocessing step (clipping
and renormalisation) described in Chapter~\ref{ch:global_pod_pipeline}.
Figure~\ref{fig:appE_phase_dynamics} displays the shift-magnitude
evolution for each benchmark regime.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../Thesis_Figures/thesis_phase_dynamics_detail.pdf}
\caption{Shift magnitude $|\delta(t)|$ (pixels) over the forecast
horizon for each of the eight benchmark regimes.  Constant-speed
regimes (top row) show smooth quasi-periodic oscillations;
variable-speed variants (bottom row) exhibit faster, noisier
fluctuations that are harder to predict with a fixed-lag AR model.}
