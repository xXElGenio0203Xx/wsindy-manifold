...
\caption{Synthetic 1D illustration of the closure problem,
...
The closure problem is primarily a \emph{rollout} problem, not an
identification problem.  Poor autonomous PDE rollout is therefore not
interpreted first as weak-form identification failure, but as evidence that
the current state variables and candidate library do not provide a
sufficiently closed effective description at the chosen coarse-graining
scale.  WSINDy minimises a weak-form residual over
...
\paragraph{Falsification content of the identification claim.}
...
\texttt{supernova}     & 0.999   & 9 \\
\texttt{crystal}       & 0.9999  & 9 \\
\texttt{pure\_vicsek}  & 0.99997 & 9 \\
\rowcolor{gray!10}
\quad\textit{median} & 0.9998  & --- \\
\midrule
...
(column ``Selected~$\boldsymbol{\ell}^*$'').  In preliminary
...
results.  WSINDy identification on these parameter sweeps is in
progress; preliminary EF-ROM forecast tables are reported in
Appendix~\ref{app:studies_in_progress}
