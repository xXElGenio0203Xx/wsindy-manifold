...
\texttt{NDYN06\_supernova}     & 0.999 & 0.683 & 0.508 & 9 \\
\texttt{NDYN07\_crystal}       & 0.9999 & 0.996 & ---\textsuperscript{\dag} & 9 \\
\texttt{NDYN08\_pure\_vicsek}  & 0.99997 & 0.654 & ---\textsuperscript{\dag} & 9 \\
\rowcolor{gray!10}
\quad\textit{CS\,+\,PV median} & 0.9998 & 0.994 & 0.992\textsuperscript{\ddag} & --- \\
\midrule
\multicolumn{5}{l}{\textit{VS diagnostic}} \\
\texttt{NDYN04\_gas\_VS}       & 0.999 & 0.558 & $-0.533$ & 12 \\
\texttt{NDYN05\_blackhole\_VS} & 0.952 & $-0.433$ & $-0.656$ & 10 \\
...
\rowcolor{gray!10}
\quad\textit{VS median}        & 0.998 & 0.206 & $-0.595$\textsuperscript{\S} & --- \\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize \textsuperscript{\dag}LSTM forecast $R^2$ pending
dedicated LSTM-only retraining (crystal, crystal\_VS, pure~Vicsek,
supernova\_VS).
\textsuperscript{\ddag}LSTM CS\,+\,PV median over gas, blackhole,
supernova (3 of 5); crystal and pure~Vicsek pending.
\textsuperscript{\S}VS LSTM median over gas\_VS and blackhole\_VS
(2 of 4); supernova\_VS and crystal\_VS pending.}
\end{table}
...
Noise-sweep WSINDy identification results are pending (Tier~2 WSINDy
step in progress on OSCAR); the data below report the companion MVAR and
LSTM forecast quality at each noise level, providing context for the
identification analysis once available.

\begin{table}[ht]
\centering
\caption{Noise-sweep forecast $R^2$ (mean over four IC families).
WSINDy identification metrics (weak-form $R^2$, $|\mathcal{A}|$, backbone
stability) are pending.  Native noise levels are marked with~$\star$.}
\label{tab:noise_sweep_forecast}
\renewcommand{\arraystretch}{0.95}
\small
\begin{tabular}{llcc}
\toprule
\textbf{Regime} & $\boldsymbol{\eta}$ & \textbf{MVAR} $R^2$ & \textbf{LSTM} $R^2$ \\
\midrule
Gas      & 0.50          & 0.993 & 0.972 \\
Gas      & 1.50$\star$   & 0.994 & 0.977 \\
Gas      & 2.00          & 0.994 & 0.806 \\
\midrule
Blackhole & 0.05$\star$  & 0.995 & 0.988 \\
Blackhole & 0.15         & 0.986 & 0.966 \\
Blackhole & 1.00         & \multicolumn{2}{c}{\textit{pending}} \\
\bottomrule
\end{tabular}
\end{table}

MVAR forecast quality is remarkably stable across the tested noise range
($R^2 \geq 0.986$ in all completed cases), suggesting that the POD basis
and autoregressive dynamics are robust to moderate noise perturbations.
LSTM degrades more noticeably at the highest gas noise level
($\eta{=}2.0$: $R^2{=}0.806$ vs.\ $0.977$ at native $\eta{=}1.5$),
consistent with the sensitivity to distributional shift discussed in
Section~\ref{sec:ch7_lstm_modes}.
If the WSINDy identification step confirms backbone stability, this will
provide evidence that the identified PDEs reflect genuine mean-field
structure rather than over-fitting to a single noise realisation.
...
The $N$-convergence WSINDy identification step is pending (Tier~4
WSINDy jobs queued on OSCAR); the coefficient-convergence figure will
be added once those results are available.  In the meantime,
Table~\ref{tab:n_convergence_forecast} reports MVAR and LSTM forecast
quality at each particle count, providing complementary evidence on how
the ROM pipeline scales with $N$.
Preliminary identification at $N{=}100$ (the thesis-default)
...
possibly due to increased latent dimensionality.  WSINDy identification
metrics are pending.}
...
\begin{tabular}{rccc}
\toprule
$N$ & \textbf{MVAR} $R^2$ & \textbf{LSTM} $R^2$ & \textbf{MVAR} $R^2$ (Gaussian IC) \\
\midrule
  50  & 0.635 &  0.229 & 0.982 \\
 100  & 0.542 &  0.085 & 0.992 \\
 200  & 0.647 & $-0.794$ & 0.995 \\
 300  & 0.654 & $-0.914$ & 0.992 \\
 500  & \multicolumn{3}{c}{\textit{pending}} \\
1000  & \multicolumn{3}{c}{\textit{pending}} \\
\bottomrule
\end{tabular}
\end{table}

MVAR mean $R^2$ is stable at $0.54$--$0.65$ across the tested range,
and reaches $R^2 > 0.98$ on the Gaussian IC family at all particle
counts---indicating that the forecast ceiling is limited by IC diversity
rather than particle number.  LSTM forecast quality degrades markedly
from $R^2{=}0.23$ at $N{=}50$ to $R^2{=}-0.91$ at $N{=}300$, likely
reflecting over-fitting to the growing latent state dimension.
