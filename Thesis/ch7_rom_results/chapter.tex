...
Because the sPOD alignment step removes bulk translation before latent-space
modelling, the forecast task studied here is not full transport
...
$R^2{=}0.995$) and remains positive on every VS regime (median
...
\quad Median test $R^2$           & 0.995        & 0.585         \\
...
correction applied only at evaluation time~\cite{gillis2019neareststable}: it neutralises the few
...
\paragraph{LSTM overhead without forecast gain.}
...
extract structural information about which mechanisms are active,
thereby asking questions that no amount of ROM forecasting can answer.
