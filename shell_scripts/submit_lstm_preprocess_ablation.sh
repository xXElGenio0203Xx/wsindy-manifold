#!/bin/bash
# ============================================================================
# Upload + Submit LSTM Preprocessing Ablation to Oscar
# ============================================================================
...
echo "======================================================================"
echo "LSTM PREPROCESSING ABLATION — OSCAR SUBMISSION"
echo "======================================================================"
echo ""

echo -e "${BLUE}Step 1: Regenerating configs/LSTM_preprocess_ablation...${NC}"
python scripts/create_lstm_preprocess_ablation_configs.py
MANIFEST_LOCAL="configs/LSTM_preprocess_ablation/manifest.txt"
...
rsync -avz --relative \
    configs/LSTM_preprocess_ablation/ \
    slurm_scripts/run_lstm_preprocess_ablation.slurm \
    scripts/create_lstm_preprocess_ablation_configs.py \
...
if [ "${DRY_RUN}" = true ]; then
    ...
    echo "  ssh ${OSCAR_HOST}"
    echo "  cd ${OSCAR_DIR}"
    echo "  sbatch slurm_scripts/run_lstm_preprocess_ablation.slurm"
    exit 0
fi
...
N=\$(wc -l < configs/LSTM_preprocess_ablation/manifest.txt | tr -d ' ')
echo "Manifest has \$N configs"

module load python/3.11.11-5e66
source ~/wsindy_env_new/bin/activate

JOB_ID=\$(sbatch ${SBATCH_ARGS} slurm_scripts/run_lstm_preprocess_ablation.slurm | grep -oP '\d+')
...
echo "  ssh ${OSCAR_HOST} 'tail -f ~/wsindy-manifold/slurm_logs/lprep_*.out'"
echo ""
echo "Download results when done:"
echo "  rsync -avz ${OSCAR_HOST}:${OSCAR_DIR}/oscar_output/LPREP_* oscar_output/"
